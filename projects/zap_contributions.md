...
**Problem:** Local BYOP providers respond to agent prompts but often do not emit native `message.tool_calls` stream events. Models emit tool-shaped JSON in assistant `content` instead, which Zap displayed but did not execute. Multi-turn follow-ups could not use prior command output.
...
**Problem:** Agent Mode requests with tools failed with HTTP 400/404 on wrong endpoints. gpt-5.x hit `/v1/chat/completions`, Ollama hit `/v1/api/chat`, and Anthropic doubled `/messages/messages`.
...
**Problem:** Settings view text editor on Linux did not support Ctrl+V paste or context-menu Cut/Copy/Paste, which made it difficult to configure BYOP API keys and base URLs.
